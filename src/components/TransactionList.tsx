...
import { ACCOUNTS, PAYMENT_MODES, SUB_CATEGORIES } from '@/lib/constants'
...
const MODE_STYLE = {
  send:    { label: 'Send', badgeCls: 'bg-red-50 text-red-700',     amtCls: 'text-red-600',     sign: '−' },
  receive: { label: 'Receive',     badgeCls: 'bg-emerald-50 text-emerald-700', amtCls: 'text-emerald-600', sign: '+' },
  ...
  const filteredTx = useMemo(() => {
    ...
    const list = transactions.filter(t => {
      ...
      return (
        ...
        (filterSubCat === 'all' || t.sub_category === filterSubCat) &&
        (filterPayMode === 'all' || t.payment_mode === filterPayMode)
      )
    })
    ...
    return list
  }, [transactions, search, filterMode, filterAccount, filterSubCat, filterPayMode, sortDateDesc])

  const filterLabel = selectedMonth || (dateFrom && dateTo ? `${dateFrom} → ${dateTo}` : dateFrom || dateTo || '')

  const totals = useMemo(() => {
    const sales     = filteredTx.filter(t => getMode(t) === 'receive').reduce((s, t) => s + t.amount, 0)
    const purchases = filteredTx.filter(t => getMode(t) === 'send').reduce((s, t) => s + t.amount, 0)
    const expenses  = filteredTx.filter(t => getMode(t) === 'expense').reduce((s, t) => s + t.amount, 0)
    ...
    <div className="px-3 sm:px-0">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-3 mb-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Search party, description, notes, sub-cat, amount…"
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        ...
        <select value={filterSubCat} onChange={e => setFilterSubCat(e.target.value)}
          ...
          {Array.from(new Set(transactions.flatMap(t => t.sub_category ? [t.sub_category] : []))).sort().map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-1 mb-2">
        <span className="text-xs text-slate-400 font-semibold">
          ...
          {filterMode !== 'all' || filterAccount !== 'all' || filterSubCat !== 'all' || filterPayMode !== 'all' || !!search || isFiltered ? ' (filtered)' : ''}
        </span>
        <div className="flex items-center gap-3 text-xs font-semibold">
          {(filterMode === 'all' || filterMode === 'receive') && totals.sales > 0 && (
            <span className="text-emerald-600">+{fmt(totals.sales)}</span>
          )}
          {(filterMode === 'all' || filterMode === 'send') && totals.purchases > 0 && (
            <span className="text-red-500">−{fmt(totals.purchases)}</span>
          )}
          {(filterMode === 'all' || filterMode === 'expense') && totals.expenses > 0 && (
            <span className="text-violet-500">−{fmt(totals.expenses)}</span>
          )}
          <button
            onClick={() => downloadTransactionExcel(filteredTx)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-colors font-bold"
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden bg-white">
            <table className="w-full text-xs" style={{tableLayout:'fixed'}}>
              ...
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="py-2.5 px-2 text-[10px] font-bold text-slate-400 uppercase tracking-wide text-left cursor-pointer select-none hover:text-slate-600 transition-colors" onClick={() => setSortDateDesc(v => !v)}>
                    Date {sortDateDesc ? '↓' : '↑'}
                  </th>
              ...
              <tbody>
                {filteredTx.map((t, idx) => {
                  ...
                  const shortLabel = mode === 'send' ? 'Send' : mode === 'receive' ? 'Receive' : 'Expense'
                  return (
...
function downloadTransactionExcel(transactions: Transaction[]) {
  import('xlsx').then(XLSX => {
    const today = new Date().toISOString().split('T')[0]

    const toRow = (t: Transaction) => ({
      'Date':            t.date ? t.date.split('-').reverse().join('/') : '',
      'Credit / Debit':  t.type === 'income' ? 'Credit' : 'Debit',
      'Account':         t.account,
      'Amount':          t.amount,
      'Category':        t.category ?? '',
      'Sub-Category':    t.sub_category ?? '',
      'Description':     t.description ?? '',
      'Company / Party': t.company_name ?? '',
      'Payment Mode':    t.payment_mode ?? '',
      'Notes':           t.notes ?? '',
    })

    const buildSheet = (list: Transaction[]) => {
      const rows = list.map(toRow)
      const credit = list.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0)
      const debit  = list.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0)
      rows.push({
        'Date': '', 'Credit / Debit': 'TOTAL', 'Account': '',
        'Amount': 0,
        'Category': `Credit: ₹${credit.toLocaleString('en-IN')} | Debit: ₹${debit.toLocaleString('en-IN')}`,
        'Sub-Category': '', 'Description': '', 'Company / Party': '', 'Payment Mode': '', 'Notes': '',
      })
      const headers = ['Date','Credit / Debit','Account','Amount','Category','Sub-Category','Description','Company / Party','Payment Mode','Notes']
      const ws = XLSX.utils.json_to_sheet(rows, { header: headers })
      ws['!cols'] = [12,14,12,14,22,22,36,28,16,30].map(wch => ({ wch }))
      return ws
    }

    const wb = XLSX.utils.book_new()

    const icici = transactions.filter(t => t.account === 'ICICI')
    const sbi   = transactions.filter(t => t.account === 'SBI')
    const cash  = transactions.filter(t => t.account === 'Cash')
    const other = transactions.filter(t => !['ICICI','SBI','Cash'].includes(t.account))

    if (icici.length > 0) XLSX.utils.book_append_sheet(wb, buildSheet(icici), 'ICICI')
    if (sbi.length   > 0) XLSX.utils.book_append_sheet(wb, buildSheet(sbi),   'SBI')
    if (cash.length  > 0) XLSX.utils.book_append_sheet(wb, buildSheet(cash),  'Cash')
    if (other.length > 0) XLSX.utils.book_append_sheet(wb, buildSheet(other), 'Other')

    // If somehow nothing matched, fall back to single sheet
    if (wb.SheetNames.length === 0) {
      XLSX.utils.book_append_sheet(wb, buildSheet(transactions), 'Transactions')
    }
